...
static void ICACHE_FLASH_ATTR sent_callback(void * arg)
{
	...
	if (req->post_data == NULL) {
		...
		if (req->secure)
			...
			espconn_sent(conn, (uint8_t *)req->post_data, os_strlen(req->post_data));
...
static void ICACHE_FLASH_ATTR connect_callback(void * arg)
{
	...
	char buf[69 + os_strlen(method) + os_strlen(req->path) + os_strlen(req->hostname) +
			 os_strlen(req->headers) + os_strlen(post_headers)];
...
static void ICACHE_FLASH_ATTR disconnect_callback(void * arg)
{
	...
	if(conn->reverse != NULL) {
		...
		if (req->buffer == NULL) {
			...
			if (os_strncmp(req->buffer, version10, os_strlen(version10)) != 0
			 && os_strncmp(req->buffer, version11, os_strlen(version11)) != 0) {
				printf("Invalid version in %s\n", req->buffer);
			}
			else {
				http_status = safe_atoi(req->buffer + os_strlen(version10));
...
void ICACHE_FLASH_ATTR http_post(const char * url, const char * post_data, const char * headers, http_callback user_callback)
{
	...
	bool is_http  = os_strncmp(url, "http://",  os_strlen("http://"))  == 0;
	bool is_https = os_strncmp(url, "https://", os_strlen("https://")) == 0;
	...
	if (is_http)
		...
		port = 443;
		secure = true;
		url += os_strlen("https://"); // Get rid of the protocol.
	} else {
		printf("URL is not HTTP or HTTPS %s\n", url);
		return;
	}
	// this is bad
	char * path = (char*)my_strchr(url, '/');
	if (path == NULL) {
		path = (char*)my_strchr(url, '\0'); // Pointer to end of string.
	}

	char * colon = (char*)my_strchr(url, ':');
	if (colon > path) {
